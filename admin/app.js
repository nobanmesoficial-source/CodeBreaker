...
async function loadData() {
  try {
    ...
    lbBody.innerHTML = leaderboard.map((u, i) =>
      `<tr>
        <td>${i + 1}</td>
        <td>${u.username}</td>
        <td>${u.level1_wins}</td>
    ...
    gamesBody.innerHTML = games.map(g =>
      `<tr>
        <td>${g.id}</td>
        <td>Level ${g.level}</td>
        <td style="font-family:monospace;color:#00E676">${g.level === 1 ? g.code : g.code.slice(0, 10) + '...'}</td>
        <td>${g.active ? '<span style="color:#00E676">Active</span>' : '<span style="color:#FF5252">Inactive</span>'}</td>
    ...
    actBody.innerHTML = activity.map(a =>
      `<tr>
        <td>${a.username}</td>
        <td>Level ${a.level}</td>
        <td style="font-family:monospace">${a.guess}</td>
        <td>${a.correct ? '<span style="color:#00E676">Yes</span>' : '<span style="color:#FF5252">No</span>'}</td>
        <td style="color:#8899AA;font-size:12px">${a.timestamp || '-'}</td>
      </tr>`
    ).join('');
  } catch (err) {
